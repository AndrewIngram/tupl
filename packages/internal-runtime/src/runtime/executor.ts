...
import { buildProviderFragmentForRelResult } from "@tupl-internal/planner";
import {
...
async function tryExecuteRemoteSubtreeResult<TContext>(
  ...
  BetterResult<QueryRow[] | null, TuplPlanningError | TuplExecutionError | TuplGuardrailError>
> {
  if (node.kind === "sql") {
...
async function executeScanResult<TContext>(
  ...
  BetterResult<InternalRow[], TuplPlanningError | TuplExecutionError | TuplGuardrailError>
> {
